        ...
        <?php if (count($all_transactions) > 0): ?>
            <ul class="trans-list" id="transactionsList">
                <?php foreach ($all_transactions as $trans): ?>
                    <li class="trans-item" data-type="<?= strtolower($trans->type) ?>" data-description="<?= htmlspecialchars($trans->description) ?>" onclick="window.location.href='transfer_success.php?ref=<?= $trans->reference ?>'">
                        <div class="trans-icon <?= strtolower($trans->type) ?>">
                            <i class="ti <?= strtolower($trans->type) === 'credit' ? 'ti-arrow-down-left' : (strtolower($trans->type) === 'top_up' ? 'ti-credit-card' : 'ti-arrow-up-right') ?>"></i>
                        </div>
